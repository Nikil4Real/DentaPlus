...
export async function fetchPatients(clinicId: string): Promise<Patient[]> {
  const { data, error } = await supabase
    .from('patients').select('*').eq('clinic_id', clinicId).order('created_at', { ascending: false });
  if (error) { console.error('fetchPatients:', error.message); return []; }
  return (data ?? []).map(r => patientFromRow(r as Record<string, unknown>));
}

export async function fetchDoctors(clinicId: string): Promise<Doctor[]> {
  const { data, error } = await supabase
    .from('doctors').select('*').eq('clinic_id', clinicId).order('name');
  if (error) { console.error('fetchDoctors:', error.message); return []; }
  return (data ?? []).map(r => doctorFromRow(r as Record<string, unknown>));
}

export async function fetchAppointments(clinicId: string): Promise<Appointment[]> {
  const { data, error } = await supabase
    .from('appointments').select('*').eq('clinic_id', clinicId).order('date', { ascending: false });
  if (error) { console.error('fetchAppointments:', error.message); return []; }
  return (data ?? []).map(r => appointmentFromRow(r as Record<string, unknown>));
}

export async function fetchLabTests(clinicId: string): Promise<DentalXRay[]> {
  const { data, error } = await supabase
    .from('dental_xrays').select('*').eq('clinic_id', clinicId).order('created_at', { ascending: false });
  if (error) { console.error('fetchLabTests:', error.message); return []; }
  return (data ?? []).map(r => xrayFromRow(r as Record<string, unknown>));
}

export async function fetchPharmacy(clinicId: string): Promise<PharmacyItem[]> {
  const { data, error } = await supabase
    .from('pharmacy_items').select('*').eq('clinic_id', clinicId).order('name');
  if (error) { console.error('fetchPharmacy:', error.message); return []; }
  return (data ?? []).map(r => pharmacyFromRow(r as Record<string, unknown>));
}

export async function fetchInvoices(clinicId: string): Promise<Invoice[]> {
  const { data, error } = await supabase
    .from('invoices').select('*').eq('clinic_id', clinicId).order('created_at', { ascending: false });
  if (error) { console.error('fetchInvoices:', error.message); return []; }
  return (data ?? []).map(r => invoiceFromRow(r as Record<string, unknown>));
}

export async function fetchPrescriptions(clinicId: string): Promise<Prescription[]> {
  const { data, error } = await supabase
    .from('prescriptions').select('*').eq('clinic_id', clinicId).order('created_at', { ascending: false });
  if (error) { console.error('fetchPrescriptions:', error.message); return []; }
  return (data ?? []).map(r => prescriptionFromRow(r as Record<string, unknown>));
}
